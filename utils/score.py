...
def calculateScore(strategy_a, strategy_b, config): 
  score_a = 0
  score_b = 0
  history_a = "";
  history_b = "";
  ...
  for x in range(config["independent_rounds"]):
    ...
    move_score_a, move_score_b = calculateMove(strategy_a[x], strategy_b[x], config)
  ...
  move_score_a, move_score_b = calculateMove(strategy_a[strategy_a_index], strategy_b[strategy_b_index], config)
  score_a += move_score_a
  score_b += move_score_b
...
def calculateMove(move_a, move_b, config):
...
def calculateScoreTournament(strategy, training_set, config):
  score = 0
  for competitor in training_set:
    scoreA, _ = calculateScore(strategy, competitor, config)
    score += scoreA
